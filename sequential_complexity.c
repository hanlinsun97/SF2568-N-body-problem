...
int main(){
	...
	// Complexity array
	double complexity[N_complexity-1];

	
	for(t=1;t<N_complexity;t++){
		int N = 50*t;
		...
		// Stocking the time spent
		complexity[t-1] = time_spent;
	...
	FILE *fp1 = fopen("/Users/pyl/Desktop/ParallelProject/complexity_param4.txt", "w");
    for (i= 0; i < N_complexity -1; i++){
        fprintf(fp1, "%f, %d\n", complexity[i], (i+1)*50);
    }
    fclose(fp1);
