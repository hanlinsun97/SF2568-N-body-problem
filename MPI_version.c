...
void recursive_doubling(int myid, double* x, double* y, double* x_new, double* y_new, double* v_x, double* v_y, int numprocess, int N, int N_p, int R_p, int tag){
	...
	if (myid < numprocess - p1){
		...
		for(i=0;i<N;i++){
			...
			if (x[i] != 0) x_new[i] = x[i];
			if (y[i] != 0) y_new[i] = y[i];
		}
	}
	if (myid < p1){
		flip = 1;
...
// Update position and velocity of particle indexed by 'i'
...
// Update the extremal boundaries of the root + reser forces
...
int main(int argc, char* argv[])
{
	...
	// index of particle and time index
	int t, i;
	int N=10;
	// Process number and number of process
	int myid, numprocess;
	...
	// Data partionning : we have NUMPROCESS, and N data : each process is going to have N_p data to work on
