...
int sphere_distribution(double min, double max, double *x_inter_1, double *y_inter_1, double *z_inter_1, double *x_inter_2, double *y_inter_2,double *z_inter_2, double x_interval, double y_interval, double z_interval, int N)
{
    //Square first
    int j = 0;
    int i = 0;
    ...
    printf("%f\n", x_interval);
    for(x_ele = min; x_ele < max;x_ele = x_ele + x_interval){  
        for (y_ele = min; y_ele < max; y_ele = y_ele + y_interval){
            for (z_ele = min; z_ele < max; z_ele = z_ele + z_interval){
                x_inter_1[i] = x_ele;
                y_inter_1[i] = y_ele;
                z_inter_1[i] = z_ele;
                printf("i = %d\n", i);
    ...
    for(int i=0;i<total;i++){
        distance = sqrt((x_inter_1[i]-center)*(x_inter_1[i]-center) + (y_inter_1[i]-center)*(y_inter_1[i]-center) + (z_inter_1[i]-center)*(z_inter_1[i]-center));
      //  printf("distance %f\n", distance);
      //  printf("%d\n",i);
        if (distance < (max - min)/2)
        {
            x_inter_2[j] = x_inter_1[i];
            y_inter_2[j] = y_inter_1[i];
            z_inter_2[j] = z_inter_1[i];
            j = j + 1;
            printf("%d, %d\n", i,j);
         }
...
void compute_force(node *father, int index, double *force_x, double *force_y, double *force_z, double *x, double *y, double *z, double parameter)
{
    ...
    if (father != NULL)
    {
        ...
        if ((r * parameter >= d) || (father->N_particle == 1))
        {
            ...
            {
                // Actualize forces
                force_x[index] = force_x[index] + sign_x / (r_x * r_x);
                force_y[index] = force_y[index] + sign_y / (r_y * r_y);
                force_z[index] = force_z[index] + sign_z / (r_z * r_z);
            }
        }
        else
        {
...
int main(int argc, char *argv[])
{
   ...
   // {
        ...
        double min = x_min;
        double max = x_max;
        double *x_inter_1 = (double *)calloc(N, sizeof(double));
        double *y_inter_1 = (double *)calloc(N, sizeof(double));
        double *z_inter_1 = (double *)calloc(N, sizeof(double));
        double *x_inter_2 = (double *)calloc(N, sizeof(double));
        double *y_inter_2 = (double *)calloc(N, sizeof(double));
        double *z_inter_2 = (double *)calloc(N, sizeof(double));
        double x_interval = 2.0;
        double y_interval = 2.0;
        double z_interval = 2.0;
    
        // Position initialization
       // random_position_distribution(x_min, x_max, y_min, y_max, z_min, z_max, x, y, z, N);
        N = sphere_distribution(min, max, x_inter_1, y_inter_1, z_inter_1, x_inter_2,y_inter_2,z_inter_2, x_interval, y_interval, z_interval, N);
